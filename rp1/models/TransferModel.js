...
class TransferModel {
    ...
    async addTransfer({ fromPSP, from, toPSP, to, amount, note, correlationId, producerStatus, consumerStatus }) {
        const [id] = await knex("transfer").insert({ fromPSP, from, toPSP, to, amount, note, correlationId, producerStatus, consumerStatus });
        return id;
    }
    ...
    async updateStatusByCorrelationId({ correlationId, statusType ,status }) {
        ...
        await knex("transfer").where({ correlationId }).update({ [statusType]: status });
    }
