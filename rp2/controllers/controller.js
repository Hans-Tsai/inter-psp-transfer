...
const createToken = ({ account, username }) => {
    ...
    return jwt.sign({ psp, account, username }, config.server2.jwt_secret, {
...
const psp2_get = (req, res) => {
    res.render("index");
};

const psp2_register_get = async (req, res) => {
    res.render("register");
};

const psp2_register_options_post = async (req, res) => {
    try {
        ...
        await redis.db5.setUserCurrentChallenge(redis.client, account, options.challenge);
...
const psp2_register_result_post = async (req, res) => {
    try {
        let { attResp, account, username } = req.body;
        account = base64url.decode(account);
        const expectedChallenge = await redis.db5.getUserCurrentChallenge(redis.client, account);
        let verification;
        verification = await SimpleWebAuthnServer.verifyRegistrationResponse({
            response: attResp,
            expectedChallenge,
            expectedOrigin: config.rp2.origin,
            expectedRPID: config.rp2.id,
        });
...
const psp2_authenticate_get = async (req, res) => {
    res.render("authenticate");
};

const psp2_authenticate_options_post = async (req, res) => {
    try {
...
const psp2_authenticate_result_post = async (req, res) => {
    try {
...
const psp2_logout_get = async (req, res) => {
...
const psp2_deposit_get = (req, res) => {
    res.render("deposit");
};

const psp2_deposit_post = async (req, res) => {
...
const psp2_withdraw_get = (req, res) => {
    res.render("withdraw");
};

const psp2_withdraw_post = async (req, res) => {
    const { account, balance, amount } = req.body;
    try {
...
const psp2_transfer_get = (req, res) => {
...
const psp2_transfer_post = async (req, res) => {
    try {
        ...
        let status = false;
        const id = await TransferModel.addTransferLog({ fromPSP, from, toPSP, to, amount, note, trxId, status });
...
const psp2_inter_psp_transfer_get = (req, res) => {
...
const psp2_inter_psp_transfer_2pc_prepare_post = async (req, res) => {
...
const psp2_inter_psp_transfer_2pc_commit_post = async (req, res) => {
    const { xaId } = req.body;
    try {
...
const psp2_inter_psp_transfer_2pc_rollback_post = async (req, res) => {
    const { xaId } = req.body;
    try {
        await knex.raw("XA ROLLBACK ?;", [xaId]);
...
module.exports = {
    psp2_get,
    psp2_register_get,
    psp2_register_options_post,
    psp2_register_result_post,
    psp2_authenticate_get,
    ...
    psp2_transfer_get,
    psp2_transfer_post,
    psp2_inter_psp_transfer_get,
    psp2_inter_psp_transfer_2pc_prepare_post,
    psp2_inter_psp_transfer_2pc_commit_post,
    psp2_inter_psp_transfer_2pc_rollback_post,
};
