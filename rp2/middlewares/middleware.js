...
const requireAuth = async (req, res, next) => {
    ...
    if (userJwt) {
        try {
            const decodedToken = await verify(userJwt, config.server2.jwt_secret);
            ...
            res.redirect("/psp2/authenticate");
        }
    } else {
...
const requireSCA = async (req, res, next) => {
    ...
    if (scaToken) {
        try {
            const decodedToken = await verify(scaToken, config.server2.jwt_secret);
            // console.log(decodedToken);
            next();
        } catch (err) {
